...
class _SIG17SRMixin(object):
    ...
    def _list_scene_dirs(self, scenes_dir):
        """List only real scene directories and ignore hidden files.

        This prevents files such as .DS_Store from being treated as SIG17
        scene folders.
        """
        return sorted([
            name for name in os.listdir(scenes_dir)
            if not name.startswith('.') and osp.isdir(osp.join(scenes_dir, name))
        ])

    def _read_training_label(self, label_path):
        """Read the training HDR label with a filename fallback.

        Cropped training data may use label.hdr, while some SIG17/Kalantari17
        layouts keep the original HDRImg.hdr name. Prefer label.hdr for
        compatibility with the original cropped-data convention, then fall back
        to HDRImg.hdr when label.hdr is absent.
        """
        label_name = 'label.hdr'
        if not osp.exists(osp.join(label_path, label_name)):
            label_name = 'HDRImg.hdr'
        return read_label(label_path, label_name)
...
class SIG17_SR_Training_Dataset(_SIG17SRMixin, Dataset):
    ...
    def __init__(self, root_dir, sub_set, is_training=True, scale=2):
        ...
        self.scenes_list = self._list_scene_dirs(self.scenes_dir)
    ...
    def __getitem__(self, index):
        ...
        label = self._read_training_label(self.image_list[index][2])
